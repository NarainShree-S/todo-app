...
public class TodoController {
    ...
    @ApiResponses(value={
            ...
            @ApiResponse(responseCode = "404",description = "Todo was notg found")
    })
    @GetMapping("/{id}")
    ...
    ResponseEntity<Todo> updateTodoById(@RequestBody Todo todo){
        return new ResponseEntity<>(todoService.updateTodo(todo),HttpStatus.OK);// better output
    }

    @DeleteMapping("/{id}")
    void deleteTodoById(@PathVariable long id){
        todoService.deleteTodoById(id);// better output
    }
}
